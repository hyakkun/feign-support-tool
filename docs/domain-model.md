...
現在の実装では、このタプル形式が `BoardState.board` の実行時正本である。一方、盤面の保存・読み込み・インポート／エクスポート機能は存在しないため、外部から読み込む既存盤面データはない。テーブルはネイティブ HTML `table` へ置換済みであり、タプル形式はテーブルライブラリの都合ではなく、既存の editor・formatter・ポップアップとの互換境界として残っている。

このため、現時点では legacy タプルと新しい `Board` の相互変換を実装しない。`Board` は将来の保存形式・ドメイン操作の目標モデルとして維持し、次のいずれかを導入すると決めた時点で変換設計に着手する。

- 盤面の保存・復元、インポート／エクスポート
- URL やファイルでの盤面共有
- `CellItem` や `RoleId` を前提とした新しい編集・検索・集計機能

変換を導入する場合は、`legacyBoardToBoard` と `boardToLegacyBoard` を専用モジュールに置く。プレイヤー・固定イベント・メモ行の種別、色、役職／死亡役職、日別の対象・行動を対象にし、`keyid` のような再描画用の値は新モデルへ持ち込まない。カタログに存在しない役職名や自由入力は情報を失わない `note` として扱う方針を、実装前にテストとともに確定する。
...
補助表示ウィンドウは、選択中の日についてイベントを各プレイヤーカード内に表示する。

```ts
type PopupPlayerEvents = Record<string, Array<"自爆" | "道連れ">>;
```

`PopupPlayerEvents`は各固定イベント行のセルに入力された人物名だけから抽出する。死亡役職が追記されていても、主情報は人物名とする。自爆・道連れを入力していないプレイヤーのカードにはイベント欄を表示しない。

対象プレイヤーのカードには「イベント: 自爆」または「イベント: 道連れ」のように表示する。同じ人物が両方の行に記録されている場合は、同一カードへ両方を表示する。実行者、追放日、ろうそくの対象、成否、死亡理由を推論・関連付けない。ポップアップは盤面の読み取り専用スナップショットから表示用の対応表を生成し、盤面データを変更しない。

将来 `Board` をポップアップへ直接渡す移植を行う場合は、`window.document`へ直接代入する既存方式ではなく、`postMessage`でバージョン付きの読み取り専用スナップショットを送る。現行のポップアップは legacy タプルを含むスナップショットを受け取っているため、この変更は変換導入時の別作業とする。
...
type DisplaySnapshot = {
  ...
  day: number;
  rows: BoardRow[];
  playerEvents: PopupPlayerEvents;
};
```
