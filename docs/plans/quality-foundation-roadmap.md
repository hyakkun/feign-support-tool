...
**状態（2026-09-13）:** Root を対象に、参加者名の設定が盤面表示と開いているポップアップ snapshot の両方へ反映されること、リセット後に同じ参加者の初期化済み盤面がポップアップへ送られることを追加で検証した。色変更が盤面とポップアップ snapshot に同期すること、自爆イベントの保存によって死亡役職 `魔術師` とポップアップのイベント表示が更新されることも検証している。`RoleSelect` が出力する役職値を Root のセル保存 callback に渡し、reducer とポップアップ snapshot が更新されることも確認した。実テーブルでは、名前変更後の自爆対象候補が最新の参加者だけになり、選択結果がセルへ保存されること、名前列のソートが翌日追加後も維持されること、色選択後に名前欄アイコンが更新されることを end-to-end で検証している。さらに popup の実 DOM で、snapshot の色・役職・対象・イベントがカードへ反映され、不正な origin のメッセージを無視することを検証した。ここに挙げた優先シナリオは完了とする。以降は不具合修正や機能追加時に、再現ケースをこの層へ追加する。
